...
try {
    ...
    if ($status === 'success') {
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE external_ref = ?
        ");
        $stmt->execute([$statusMessage, $telecomFee, $externalId]);
        
        logIotecCallback("Transaction marked as SUCCESS for externalId: $externalId - Telecom Fee (4%): $telecomFee", 'SUCCESS');
