...
class YoAPI {
    ...
    */
    public function ac_deposit_funds($msisdn, $amount, $narrative)
    {
        ...
        $msisdn = trim($msisdn);
        $amount = number_format((float)$amount, 0, '', ''); // Format as integer, no decimals
        $narrative = htmlspecialchars($narrative, ENT_XML1, 'UTF-8');
            
        $xml = '';
        $xml .= '<?xml version="1.0" encoding="UTF-8"?>';
        $xml .= '<AutoCreate>';
        $xml .= '<Request>';
        $xml .= '<APIUsername>'.htmlspecialchars($this->username, ENT_XML1, 'UTF-8').'</APIUsername>';
        $xml .= '<APIPassword>'.htmlspecialchars($this->password, ENT_XML1, 'UTF-8').'</APIPassword>';
    ...
    */
    ...
    protected function verify_payment_notification()
    {
        ...
        if (!isset($_POST['date_time'], $_POST['amount'], $_POST['narrative'], $_POST['network_ref'], $_POST['external_ref'], $_POST['msisdn'], $_POST['signature'])) {
            return false;
        }
    ...
    {
        // Check if required POST data exists
        if (!isset($_POST['failed_transaction_reference'], $_POST['transaction_init_date'], $_POST['verification'])) {
            return false;
        }
