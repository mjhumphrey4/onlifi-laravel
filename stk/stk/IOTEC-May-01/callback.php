...
try {
    ...
    if ($status === 'success') {
        ...
        // Update transaction to success with all fees and vendor details
        $stmt = $pdo->prepare("
            UPDATE transactions
            SET status = 'success',
                status_message = ?,
            ...
            AND status = 'pending'
        ");
        $stmt->execute([$statusMessage, $telecomFee, $platformFee, $vendorTransactionId ?: $transactionId, $externalId]);
