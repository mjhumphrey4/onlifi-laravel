...
export const api = {
  ...
  saveProvider: (record: any) =>
    request(record.id ? `/providers/${record.id}` : "/providers", {
  ...
  saveWithdrawalApi: (record: any) =>
    request(record.id ? `/withdrawal-apis/${record.id}` : "/withdrawal-apis", {
      method: record.id ? "PUT" : "POST",
      body: JSON.stringify(record),
    }),
  deleteWithdrawalApi: (id: number) => request(`/withdrawal-apis/${id}`, { method: "DELETE" }),
