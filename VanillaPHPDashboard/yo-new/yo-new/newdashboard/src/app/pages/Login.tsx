    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-semibold text-primary mb-1">Onlifi Lite-Edition</h1>
          <p className="text-sm text-muted-foreground">A Mobile Money Payments Dashboard That Actually Makes Sense</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-8">
          ...
          <div className="mt-6 p-4 bg-muted/40 border border-border rounded-lg">
            <p className="text-xs text-muted-foreground font-medium mb-2">Available accounts: Contact Humphrey for your Password</p>
            <div className="grid grid-cols-2 gap-1 text-xs text-muted-foreground">
              {['admin', 'enock', 'richard', 'stk', 'remmy', 'guma'].map((u) => (
                <span key={u}>
                  <span className="text-primary">{u}</span> / password
                </span>
              ))}
            </div>
          </div>
