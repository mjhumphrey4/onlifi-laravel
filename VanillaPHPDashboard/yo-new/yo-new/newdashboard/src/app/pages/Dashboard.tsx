...
import { DollarSign, TrendingUp, Wallet, RefreshCw, Receipt } from 'lucide-react';
...
export function Dashboard() {
  ...
  const availableBalance = siteList.reduce((s, [, v]) => s + v.balance, 0);
  const platformEarnings = siteList.reduce((s, [, v]) => s + (v.platform_fees || 0), 0);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {/* Summary stats */}
      <div className={`grid grid-cols-1 sm:grid-cols-2 ${isAdmin() ? 'lg:grid-cols-5' : 'lg:grid-cols-4'} gap-4 sm:gap-6 mb-6 sm:mb-8`}>
        ...
        <StatsCard title="Available Balance" value={fmt(availableBalance)} icon={Wallet} />
        <StatsCard title="Total Withdrawals" value={fmt(totalWithdrawn)} icon={Receipt} />
        {isAdmin() && <StatsCard title="Platform Earnings" value={fmt(platformEarnings)} icon={DollarSign} trend={{ value: 'Daily fees', isPositive: true }} />}
      </div>
              ...
              >
                ...
                <div className="space-y-1 text-xs bg-white/10 rounded-lg p-3">
                  ...
                  {isAdmin() && (
                    <>
                      <div className="flex justify-between">
                        <span className="opacity-80">Telecom Fees</span>
                        <span className="font-semibold text-orange-200">{fmt(stat.total_fees || 0)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="opacity-80">Platform Fees</span>
                        <span className="font-semibold text-purple-200">{fmt(stat.platform_fees || 0)}</span>
                      </div>
                    </>
                  )}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  {['Voucher', 'Phone', 'Amount', 'Telecom Fee', 'Net Amount', 'Site', 'Status', 'Date'].map((h) => (
              ...
              <tbody>
                {txs.length === 0 ? (
                  ...
                  const fee = isSuccess ? parseFloat(tx.telecom_fee || '0') : 0;
                  const platformFee = isSuccess ? parseFloat(tx.platform_fee || '0') : 0;
                  // Users see net after telecom fee only; platform fee is hidden
                  const netAmount = isAdmin() ? (amount - fee - platformFee) : (amount - fee);
                  
                  const netAmountColor = tx.status.toLowerCase() === 'success' ? 'text-primary' 
                    : tx.status.toLowerCase() === 'pending' ? 'text-yellow-500' 
                    : tx.status.toLowerCase() === 'failed' ? 'text-destructive' 
                  ...
                  return (
                    <tr key={`${tx.id}-${i}`} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
                      ...
                      <td className="py-3 px-2 sm:px-4 text-xs sm:text-sm whitespace-nowrap">
                        {isSuccess ? (
                          <span className="inline-flex items-center px-2 py-1 rounded bg-destructive/10 text-destructive font-semibold">
                            -{fmt(fee)}
                          </span>
                        ) : (
                          <span className="text-muted-foreground">—</span>
                      ...
                      <td className={`py-3 px-2 sm:px-4 text-xs sm:text-sm whitespace-nowrap font-bold ${netAmountColor}`}>{fmt(netAmount)}</td>
