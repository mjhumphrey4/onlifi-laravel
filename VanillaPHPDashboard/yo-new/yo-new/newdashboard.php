...
// --- PAGINATION, FILTERING AND SEARCH SETUP ---
$per_page = 20;
...
if (!empty($search_query)) {
    $search_condition = " AND (msisdn LIKE :search OR voucher_code LIKE :search)";
...
<body class="bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-blue-500 via-purple-500 to-green-500 flex">
                <a href="?tab=all<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?>" class="tab-button text-white <?php echo $active_tab === 'all' ? 'active' : ''; ?>">
                    All Sites
                </a>
                <a href="?tab=enock<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?>" class="tab-button text-white <?php echo $active_tab === 'enock' ? 'active' : ''; ?>">
                    Enock
                </a>
                <a href="?tab=richard<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?>" class="tab-button text-white <?php echo $active_tab === 'richard' ? 'active' : ''; ?>">
                    Richard
                </a>
                <a href="?tab=stk<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?>" class="tab-button text-white <?php echo $active_tab === 'stk' ? 'active' : ''; ?>">
                    STK
                </a>
            </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                <form method="GET" action="" class="flex gap-3">
                    ...
                    <div class="search-container flex-1">
                        ...
                        <input 
                            ...
                            class="search-input w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        >
                    ...
                    <?php if (!empty($search_query)): ?>
                    <a href="?tab=<?php echo htmlspecialchars($active_tab); ?>" class="px-6 py-2 bg-gray-500 text-white font-semibold rounded-lg hover:bg-gray-600 transition-colors">
                        Clear
                    </a>
                    <?php endif; ?>
                </form>
                ...
                <p class="mt-2 text-sm text-gray-600">
                    ...
                    <?php if (!empty($logs)): ?>
                        (<?php echo count($logs); ?> result<?php echo count($logs) !== 1 ? 's' : ''; ?> found)
                    <?php endif; ?>
                </p>
                <?php endif; ?>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                                <?php if (!empty($search_query)): ?>
                                    No results found for "<?php echo htmlspecialchars($search_query); ?>". Try a different search term.
                                <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                <?php 
                                $voucher = htmlspecialchars($log['voucher_code'] ?? 'N/A');
                                if (!empty($search_query) && stripos($voucher, $search_query) !== false) {
                                    $voucher = str_ireplace($search_query, '<mark class="bg-yellow-200">' . htmlspecialchars($search_query) . '</mark>', $voucher);
                                }
                                echo $voucher;
                                ?>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                <?php 
                                $phone = htmlspecialchars($log['msisdn']);
                                if (!empty($search_query) && stripos($phone, $search_query) !== false) {
                                    $phone = str_ireplace($search_query, '<mark class="bg-yellow-200">' . htmlspecialchars($search_query) . '</mark>', $phone);
                                }
                                echo $phone;
                                ?>
                            </td>
            ...
            <!-- Pagination with Tab and Search Preservation -->
            <?php if ($has_more_logs && $last_log_time_for_next): ?>
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 text-center">
                <a href="?tab=<?php echo urlencode($active_tab); ?>&last_log_time=<?php echo urlencode($last_log_time_for_next); ?><?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?>" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-blue-600 rounded-md hover:bg-blue-700 transition-colors inline-block">
                    Next Logs &rarr;
                </a>
            </div>
    ...
    <!-- Auto-refresh script (only on the first page with 'all' tab and no search) -->
    <script>
        <?php if (!$last_log_time && $active_tab === 'all' && empty($search_query)): ?>
        setTimeout(function() {
            location.reload();
        }, 60000);
        <?php endif; ?>
        ...
        function clearSearch() {
            ...
            window.location.href = '?tab=<?php echo htmlspecialchars($active_tab); ?>';
        }
