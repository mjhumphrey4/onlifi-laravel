...
try {
    ...
    // Third database connection for Guma
    $pdo_guma = new PDO("mysql:host=$host;dbname=guma_mikrotik;charset=utf8mb4", $username, $password);
...
// Fetch logs from omada database
$stmt1 = $pdo->prepare("
    SELECT 
        id,
        external_ref,
...
// Fetch logs from payment_mikrotik database
...
$stmt3 = $pdo_guma->prepare("
    ...
    LIMIT :limit
");
$stmt3->bindValue(':limit', $per_page + 1, PDO::PARAM_INT);
foreach ($params as $key => $value) {
...
// Merge and sort logs
$all_logs = array_merge($logs1, $logs2, $logs3);
...
foreach ($sites as $site) {
    if ($site === 'STK') {
        ...
        $salesData[$site] = getSalesDataBySite($pdo_guma, $dbSites[$site]);
    } else {
