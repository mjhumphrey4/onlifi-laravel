...
export function ImportVouchers() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedSite?.id) { setError('Please select a site.'); return; }
    setLoading(true);
    setError('');
    setResult(null);
    try {
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading || !file || !selectedSite?.id}
        ...
        <div className="mt-6 p-4 bg-muted/50 border border-border rounded-lg">
          ...
          <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
            ...
            <li>Codes are imported directly into <strong className="text-card-foreground">{selectedSite?.name || 'your site'}</strong>'s database</li>
          </ul>
        </div>
      </div>
    </div>
